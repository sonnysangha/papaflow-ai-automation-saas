...
- **A tool result with `retryable: false` is the end of the turn.** When a tool answers with
  ...
  backend is unreachable, name the setting the `message` names if it names one, and say the
  workflow was not changed. Then stop. Do not paste the `message` in whole, do not call that tool
  again in this turn, do not try a different tool to work around it, and do not apologise at length.
