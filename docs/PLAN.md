...
|---|---|---|---|
| Free — default org plan | `core_connectors` | 3 workflows · 100 runs/month · 1 member · schedules no faster than hourly | Canvas, HTTP, Resend email, Discord webhook, Telegram, one AI connection, Manual/Webhook/Form triggers. |
...
Clerk sends Svix-signed billing webhooks. A Convex `httpAction` at `/clerk-webhook` verifies with the `svix` package and upserts three tables: `organizations` (from `organization.*`), `memberships` (from `organizationMembership.*`), and `orgPlans` from `subscriptionItem.active | canceled | ended | pastDue`, keyed on `payer.organization_id`, storing `plan.slug`, `status`, `period_end` and `is_free_trial`. A cancelled item keeps its features until `period_end`; `ended` reverts to Free. For anything outside a request (the workflow engine), `clerkClient.billing.getOrganizationBillingSubscription(orgId)` exists in `@clerk/backend`, but the synced row is faster and doesn't fail when Clerk is slow.
...
Second directory, `agents/builder/`, with its own `instructions.md` (how to design a good workflow: one trigger, templates over hardcoding, prefer Extract + Switch over long prompts, always validate before finishing) and a `skills/` folder with one markdown file per connector describing what its node is good for. Tools are static files under `agent/tools/`, which they have to be for durable tools anyway. Gating happens twice: the route that opens a Builder session checks `has({ feature: "org:ai_builder" })`, and every tool's `execute` reads the org from `ctx.session.auth` and checks its plan row in Convex before doing anything (defence in depth, and it also filters `list_node_types` to what the plan allows). `defineDynamic` supplies the per-org instructions and the model: the org's own AI connection or, if they haven't added one yet, a house model on your key with a per-org cap, which is itself a reason to upgrade. Sessions are durable and multi-turn, so "actually, make it post to Discord instead" is just the next message in the same session.
...
| Google Gemini | `AIza…` | `x-goog-api-key` | `GET /v1beta/models` | `@ai-sdk/google` · factory is `createGoogle` in v7 (old name kept as alias, check the export) | `gemini-3.7-flash`, `gemini-3.1-pro-preview`, `gemini-3.1-flash-image` | Text, tools, vision, native image gen, native TTS |
...
| fal.ai | no prefix | `Authorization: Key` | cheap `flux/schnell` call | `@ai-sdk/fal` · `generateImage` | `fal-ai/flux/dev`, `recraft/v3`, Wizper STT | Images, STT |
...
| Microsoft Teams — Post to a channel | `Workflow URL` | Power Automate URL with its `sig` | `POST` Adaptive Card JSON | `no` | Office 365 connectors were shut down May 2026. User creates a "post to channel when a webhook is received" flow and pastes the URL. Payload must be an Adaptive Card, plain text no longer works. |
...
| Linear — Create issue | `API key` `OAuth` | `lin_api_…` and chosen `teamId` | `POST api.linear.app/graphql` `issueCreate`, header is the bare key, no Bearer | `push` | Validate with `{ viewer { id } }`. Webhooks need a workspace admin or an OAuth app with `admin` scope, so the trigger is v2. |
...
export async function scheduler({ scheduleId, cron }: { scheduleId: string; cron: string }) {
  ...
  await continueAsNew({ scheduleId, cron });                    // "use step": start(scheduler, …) then this run ends
}
```
...
5. **Before every node run, get a fresh token** — If `expiresAt − 60s` has passed, refresh; if the provider rotated the refresh token (Airtable, Notion) overwrite the pair. On `invalid_grant`, mark the connection "needs reconnect" and fail the step with that message.
...
// aad = `${userId}:${connectionId}`   wrong row = GCM tag check fails = throws
```
...
**Limits.** 10,000 steps and 25,000 events per run, 240 seconds max replay, 50 MB payload, no maximum duration. A Loop over 3,000 Airtable rows should run as a child workflow per chunk via `start()` from a step, then wait on a hook. Step names are compile-time (`step//run-node.ts//runNode`), so every node shows as `runNode` in the trace viewer; put `nodeId` and `nodeType` first in the step args so the trace is readable.
...
**Setup.** `npm i workflow@beta` (5.x for multi-region and `@ai-sdk/workflow`), `withWorkflow(withEve(nextConfig))`, exclude `.well-known/workflow/` and `/eve/` from the Clerk proxy matcher. Local dev uses the Local World in `.workflow-data/` with `npx workflow web` as a run inspector. On Vercel, Fluid compute on, build command `npx convex deploy --cmd 'npm run build'` with a production deploy key and a separate preview key so every PR gets its own Convex deployment. Hobby includes 50k workflow events a month, which is plenty for the shoot.

**Convex and Clerk.** Activate the Convex integration in the Clerk dashboard so the session token itself carries `aud: "convex"`; `ConvexProviderWithClerk` then sends that token and re-authenticates when the active org changes, which re-runs every subscribed query for the new workspace. In Convex functions, `ctx.auth.getUserIdentity()` exposes the token's claims, including the org object and the plan and feature claims; add readable `org_id` and `org_role` claims in the session-token customiser and use those. Convex verifies the claims pass-through from source rather than a doc that names them, so log the identity once in dev before relying on a specific key.

## The Agent node and eve

eve is Vercel's open-source agent framework (Apache-2.0, launched June 2026, still labelled beta). Two things about it shape the plan. First, it isn't a library you call from a route: an agent is a directory (`agent/instructions.md`, `agent/agent.ts`, `agent/tools/*.ts`, skills, connections) that compiles to its own service, deployed beside the Next.js app via `withEve(nextConfig)` and reached over HTTP at `/eve/v1/*`. Second, every eve turn already runs as a durable Workflow SDK run, so the Agent node gets retries and replay without doing anything.

### How the node plugs in

The Agent node's step uses `eve/client`: `client.sessions.create({ message, outputSchema })` with the user's Clerk id in the auth header, awaits the response, and returns the structured result. Per-user tools come from `defineDynamic` in the agent, which on `session.started` reads `ctx.session.auth`, loads that user's active connections, and returns tools like `slack_post`, `notion_create_page`, `telegram_send`. Tool closures must be JSON-serializable, so each tool's `execute` receives a `connectionId` and decrypts inside, exactly like `runNode`. The model is per session too: `defineDynamic` can return an AI SDK model built from the user's key, or a Gateway string with BYOK.
...
- *The plan* · **eve for the Agent node** — _Separate service, Node 24, AI SDK 7_. Right fit for the story: durable agent, per-user tools, approvals, MCP. Pin the version the day you shoot; it moved from 0.47 to 0.49 in the two days before this was written, and the launch blog's `needsApproval` is already `approval: always()` in the docs.
...
- *Fallback B* · **WorkflowAgent inside the workflow** — _@ai-sdk/workflow, needs workflow 5 beta_. Durable per model call, tools with `"use step"` executes, `needsApproval` support. Closest to eve's durability without the service, but dynamically built tool maps aren't shown in the docs; spike it first.
...
- **Workflow SDK**: Step arguments and return values are recorded in the run log and visible in the dashboard. Pass `connectionId`, never a decrypted key. `"use workflow"` code can't use global `fetch`, timers, `Buffer` or Node modules; all I/O goes in steps.
- **Workflow SDK**: `start()` inside a workflow must be called from a step. `createHook` must be called in the workflow, not a step. Renaming `runNode` or moving its file changes the step id and breaks in-flight runs on deploy.
- **eve**: Beta, Node 24+, versions daily. Pin it. It's a separate service, so local dev runs the eve dev server beside `next dev`; make sure both are up before the take. Durable tools (the ones that can `ask()`) must be static files in `agent/tools/`, never returned from `defineDynamic`.
...
- **Teams**: Old incoming webhooks died in May 2026. Workflows URL + Adaptive Card or nothing.
- **Schedules**: A once-a-minute scheduler hits the 25k-events-per-run cap in about a week. Continue-as-new every 500 iterations.
