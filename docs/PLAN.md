...
The Agent node's step uses `eve/client`: `client.sessions.create({ message, outputSchema })` with a bearer token that the agent's `channels/eve.ts` authenticator verifies with `@clerk/backend` and maps to `ctx.session.auth.current.attributes.orgId` (production fails closed without an authenticator), awaits the response, and returns the structured result. Per-user tools come from `defineDynamic` in the agent, which on `session.started` reads `ctx.session.auth`, loads that user's active connections, and returns tools like `slack_post`, `notion_create_page`, `telegram_send`. Tool closures must be JSON-serializable, so each tool's `execute` receives a `connectionId` and decrypts inside, exactly like `runNode`. The model is per session too, with one rule: `session.started`/`turn.started` handlers return model-ID strings; a live AI SDK model built from the org's key may only be returned from `step.started`.
